...
export function Game() {
  const { quiz, currentQuestionIndex, submitAnswer, lastAnswerId, status, currentPlayer } = useGameStore();
  ...
  if (status === 'FINISHED') {
      return (
          <div className="flex flex-col items-center justify-center h-full text-center space-y-6 animate-in zoom-in">
              ...
              <Button size="lg" className="mt-8" onClick={() => window.location.href = '/'}>Back to Home</Button>
          </div>
      )
  }
  ...
  if (status === 'REVIEW') {
      const isCorrect = lastAnswerId 
        ? question.options.find(o => o.id === lastAnswerId)?.isCorrect 
        : false;
    ...
    <div className="flex flex-col h-full animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 gap-3 flex-1 content-start">
        {question.options.map((option, idx) => (
            <Button
                key={option.id}
                variant={lastAnswerId === option.id ? "default" : "outline"}
                className={cn(
